defmodule Core.JwxtExtractor.Back do
  @doc """
  ...
  def extract_personal_info(cookie) do
    request_body = {:form, []}
  ...
  def list_all_dates(uid) do
    start_year = "20#{uid |> String.slice(1,2)}" |> String.to_integer
    end_year = Date.utc_today().year
    Enum.map(start_year..end_year,
      fn year ->
        [
          "#{year}-#{year + 1}-1",
          "#{year}-#{year + 1}-2",
  ...
  def extract_single_page(cookie, date) do
    request_body = {:form, [{"kksj", date}]}
    request_headers = [{"Cookie", cookie}]
    {:ok, %{body: body}} = HTTPoison.post @term_url, request_body, request_headers
    body
